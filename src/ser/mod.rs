...
pub enum Error {
    /// Buffer is full
    BufferFull,
    /// The serializing value type are not supported
    InvalidValue,
}
...
{
    Ok(unsafe { str::from_utf8_unchecked(&to_vec::<T>(value)?) }.into())
}
...
mod tests {
    ...
    #[test]
    fn array() {
        let buf = crate::to_vec(&[0, 1, 2]).unwrap();
        assert_eq!(buf.len(), 7);
    ...
    #[test]
    fn bool() {
        let buf = crate::to_vec(&true).unwrap();
        assert_eq!(buf.len(), 4);
